...
export function createNewMetaV1Event(sender: string, subject: Bytes, meta: Bytes, transactionHash: string, transactionBlockNumber: number, transactionTimestamp: number): MetaV1_2 {
  ...
  metaV1Event.block.number = BigInt.fromString(Math.floor(transactionBlockNumber).toString());
  metaV1Event.block.timestamp = BigInt.fromString(Math.floor(transactionTimestamp).toString());
